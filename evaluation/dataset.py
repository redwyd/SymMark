...
class BaseDataset:
    ...
    def get_watermark_text(self, index):
        """Return the watermark text at the specified index."""
        return self.watermark_texts[index]

    def get_unwatermark_text(self, index):
        """Return the unwatermark text at the specified index."""
        return self.unwatermark_texts[index]
...
class C4Dataset(BaseDataset):
    ...
    def load_data(self):
        ...
        for line in lines[:self.max_samples]:
            ...
            self.watermark_texts.append(item['watermark_text'])
            self.unwatermark_texts.append(item['unwatermark_text'])


class WMT16DE_ENDataset(BaseDataset):
    """Dataset class for WMT16 DE-EN dataset."""

    def __init__(self, data_source: str, max_samples: int = 500) -> None:
        """
