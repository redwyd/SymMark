...
from exceptions.exceptions import InvalidTextSourceModeError, InvalidDirectAnalyzerTypeError, \
    InvalidReferencedAnalyzerTypeError
from evaluation.tools.text_quality_analyzer import (TextQualityAnalyzer, DirectTextQualityAnalyzer,
...
class TextQualityAnalysisPipeline:
    ...
    def _get_watermarked_text(self, watermark: BaseWatermark, index: int):
        ...
        return self.dataset.get_watermark_text(index)
    ...
    def _get_unwatermarked_text(self, watermark: BaseWatermark, index: int):
        ...
        if self.unwatermarked_text_source == 'natural':
            ...
            return self.dataset.get_unwatermark_text(index)
    ...
    def evaluate(self, watermark: BaseWatermark):
        ...
        for index in bar:
            ...
            edited_unwatermarked_text = self._edit_unwatermarked_text(unwatermarked_text,
                                                                      self.dataset.get_prompt(index))

            # Prepare input for quality analyzer
            prepared_data = self._prepare_input_for_quality_analyzer(edited_watermarked_text, edited_unwatermarked_text,
                                                                     index)
            ...
            evaluation_result.append(TextQualityComparisonResult(edited_watermarked_text, edited_unwatermarked_text,
                                                                 watermarked_quality_score,
                                                                 unwatermarked_quality_score))
        ...
        if self.return_type == QualityPipelineReturnType.FULL:
            ...
            return {'watermarked': sum([result.watermarked_quality_score for result in evaluation_result]) / len(
                evaluation_result),
                    'unwatermarked': sum([result.unwatermarked_quality_score for result in evaluation_result]) / len(
                        evaluation_result)}
