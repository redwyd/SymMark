...
class WatermarkDetectionPipeline:
    ...
    def _edit_text(self, text: str, prompt: str = None, is_watermarked: bool = True) -> str:
        """Edit text using text editors."""
        for text_editor in self.text_editor_list:
            text = text_editor.edit(text, prompt, is_watermarked)
        return text
    ...
    def _detect_watermark(self, text: str, watermark: BaseWatermark, *args):
        """Detect watermark in text."""
        detect_result = watermark.detect_watermark(text, True, args[0])
    ...
    def evaluate(self, watermark: BaseWatermark, watermark_type='I'):
        ...
        for index in bar:
            ...
            detect_result = self._detect_watermark(edited_text, watermark, watermark_type)
...
class WatermarkedTextDetectionPipeline(WatermarkDetectionPipeline):
    ...
    def _generate_or_retrieve_text(self, dataset_index, watermark):
        ...
        return self.dataset.get_watermark_text(dataset_index)
...
class UnWatermarkedTextDetectionPipeline(WatermarkDetectionPipeline):
    ...
    def _generate_or_retrieve_text(self, dataset_index, watermark):
        ...
        if self.text_source_mode == 'natural':
            ...
            return self.dataset.get_unwatermark_text(dataset_index)
    ...
    def evaluate(self, watermark: BaseWatermark, watermark_type='I'):
        ...
        for index in bar:
            generated_or_retrieved_text = self._generate_or_retrieve_text(index, watermark)
            edited_text = self._edit_text(generated_or_retrieved_text, self.dataset.get_prompt(index), is_watermarked=False)
            detect_result = self._detect_watermark(edited_text, watermark, watermark_type)
            evaluation_result.append(WatermarkDetectionResult(generated_or_retrieved_text, edited_text, detect_result))
