...
#
#     def edit(self, text: str, prompt: str, backtrack_patience: int = 100, max_attempts: int = 1000, is_watermarked=True):
...
class DipperParaphraser(TextEditor):
    ...
    def edit(self, text: str, reference: str, is_watermarked=True):
...
class TruncateLLaMA3TaskTextEditor(TextEditor):
    ...
    def edit(self, text: str, reference=None, is_watermarked=True):
        ...
        if reference is not None:
            truncated_text = text[len(reference) - len("<|begin_of_text|><|start_header_id|>" + "<|end_header_id|>" + "<|eot_id|><|start_header_id|>" + "<|end_header_id|>"):]
...
class BackTranslationTextEditor(TextEditor):
    ...
    def __init__(self, device, from_lang='en', to_lang='zh') -> None:
        """
        ...
        self.model = T5ForConditionalGeneration.from_pretrained(
            '/data/wangyidan/model/t5_translate_en_ru_zh_small_1024').to(self.device)
        self.tokenizer = T5Tokenizer.from_pretrained('/data/wangyidan/model/t5_translate_en_ru_zh_small_1024')
        self.prefix1 = 'translate to zh'
        self.prefix2 = 'translate to en'

    def edit(self, text: str, reference=None, is_watermarked=True):
        intermediary_input_ids = self.tokenizer(self.prefix1 + text, return_tensors='pt')['input_ids'].to(self.device)
        intermediary_tokens = self.model.generate(intermediary_input_ids)
        intermediary_result = self.tokenizer.batch_decode(intermediary_tokens, skip_special_tokens=True)

        edit_input_ids = self.tokenizer(self.prefix2 + intermediary_result[0], return_tensors='pt')['input_ids'].to(
            self.device)
        edit_tokens = self.model.generate(edit_input_ids)
        edit_result = self.tokenizer.batch_decode(edit_tokens, skip_special_tokens=True)
        return edit_result


class CopyPasteTextEditor(TextEditor):
    """Copy watermarked text and paste it in unwatermarked text."""
    ...
    def edit(self, text: str, reference=None, is_watermarked=True):
        if is_watermarked:
            with open('/data/wangyidan/Paper2Code/Ship/dataset/c4/c4.jsonl', 'r') as f:
                lines = f.readlines()
                line = random.choice(lines)
                line = json.loads(line)

                unwatermarked_text = line['prompt'] + line['natural_text']
                ratio = int(len(text)/3)
                edit_text = unwatermarked_text + text[:ratio] + unwatermarked_text + text[ratio:2*ratio] + unwatermarked_text + text[2*ratio:] + unwatermarked_text
                # edit_text = unwatermarked_text + text
        else:
            edit_text = text
        # edit_text = text + reference * self.times
        return edit_text