...
from evaluation.pipelines.quality_analysis import DirectTextQualityAnalysisPipeline, QualityPipelineReturnType, \
    ReferencedTextQualityAnalysisPipeline, ExternalDiscriminatorTextQualityAnalysisPipeline

device = 'cuda' if torch.cuda.is_available() else 'cpu'


def assess_quality(algorithm_name, metric, transformers_config, eval_file, unwatermarked_text_source='natural'):
    if metric == 'PPL':
        my_dataset = C4Dataset(eval_file)
        pipeline = DirectTextQualityAnalysisPipeline(
            dataset=my_dataset,
            watermarked_text_editor_list=[TruncatePromptTextEditor()],  # [TruncatePromptTextEditor()]
            unwatermarked_text_editor_list=[],
            analyzer=PPLCalculator(
                model=AutoModelForCausalLM.from_pretrained("/data/wangyidan/model/llama2-7b-chat-hf", device_map='auto'),
                tokenizer=LlamaTokenizer.from_pretrained("/data/wangyidan/model/llama2-7b-chat-hf"), device=device),
            unwatermarked_text_source=unwatermarked_text_source, show_progress=True,
        ...
        transformers_config = TransformersConfig(
            model=AutoModelForCausalLM.from_pretrained("/data2/shared_model/facebook/opt-1.3b/").to(device),
        ...
        tokenizer = AutoTokenizer.from_pretrained("/data2/shared_model/facebook/nllb-200-distilled-600M/",
                                                  src_lang="deu_Latn")
        transformers_config = TransformersConfig(
            model=AutoModelForSeq2SeqLM.from_pretrained("/data2/shared_model/facebook/nllb-200-distilled-600M/").to(
                device),
            tokenizer=tokenizer,
            device=device,
            vocab_size=256206,
        ...
        transformers_config = TransformersConfig(
            model=AutoModelForCausalLM.from_pretrained("/data2/shared_model/starcoder/", device_map='auto'),
